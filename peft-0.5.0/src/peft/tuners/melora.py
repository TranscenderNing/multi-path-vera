...
class LoraLayer(BaseTunerLayer):
    def __init__(self, in_features: int, out_features: int, **kwargs):
        ...
        self.lora_aa =  nn.ParameterDict({})
        self.lora_B = nn.ModuleList([])
        self.lora_bb =  nn.ParameterDict({})
        # For Embedding layer
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, mode, init_lora_weights):
        ...
        for i, rank in enumerate(r):
            ...
            if rank > 0:
                ...
                self.lora_aa[str(i)] = nn.Parameter(torch.ones(rank))
                self.lora_bb[str(i)] = nn.Parameter(torch.randn(self.out_features))
                ...
                # nn.init.ones_(self.lora_a[i].weight)
                # nn.init.ones_(self.lora_b[i].weight)

                nn.init.constant_(self.lora_aa[str(i)], 0.001)
                nn.init.zeros_(self.lora_bb[str(i)])
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            for i, rank in enumerate(self.r[self.active_adapter]):
                if rank > 0:
                    # temp.append(self.lora_b[i](self.lora_B[i](
                    #     self.lora_a[i](self.lora_A[i](self.lora_dropout[i](x[:,:,i*(self.in_features):(i+1)*(self.in_features)])))
                    #     ))
                    #     * self.scaling[i]
                    # )
                    temp.append(self.lora_bb[str(i)] * self.lora_B[i](
                        self.lora_aa[str(i)] * self.lora_A[i](self.lora_dropout[i](x[:,:,i*(self.in_features):(i+1)*(self.in_features)]))
                        )
                        * self.scaling[i]
                    )
